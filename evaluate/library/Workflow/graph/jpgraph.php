...
// Must end with '/'
DEFINE("CACHE_DIR",PROJECT_LIBRARY_PATH."Workflow/graph/tmp/jpgraph_cache/");
...
DEFINE("TTF_DIR",PROJECT_LIBRARY_PATH."Workflow/graph/font_th/thai-ttf-0.4.3/");
...
// system setup. Note that cache functionality is turned off by
...
DEFINE("FF_LOMA",19);
DEFINE("FF_NORASI",20);
DEFINE("FF_TLWGMONO",21);
DEFINE("FF_TLWGTYPEWRITER",22);
//
...
// PRIVATE METHODS
	...
	$e = explode('.',$this->background_image);
...
// CONSTRUCTOR
	...
	$this->font_files=array(
	    // Thai Fonts.
	    FF_GARUDA =>  array(FS_NORMAL=>'Garuda',  FS_BOLD=>'Garuda-Bold',FS_ITALIC=>'Garuda-Oblique',   FS_BOLDITALIC=>'Garuda-BoldOblique' ),
	    FF_LOMA =>    array(FS_NORMAL=>'Loma',    FS_BOLD=>'Loma-Bold',  FS_ITALIC=>'Loma-Oblique',   FS_BOLDITALIC=>'Loma-BoldOblique' ),
	    FF_NORASI =>    array(FS_NORMAL=>'Norasi',    FS_BOLD=>'Norasi-Bold',  FS_ITALIC=>'Norasi-Italic',   FS_BOLDITALIC=>'Norasi-BoldItalic' ),
	    FF_TLWGMONO =>    array(FS_NORMAL=>'TlwgMono',    FS_BOLD=>'TlwgMono-Bold',  FS_ITALIC=>'TlwgMono-Oblique',   FS_BOLDITALIC=>'TlwgMono-BoldOblique' ),
	    FF_TLWGTYPEWRITER =>    array(FS_NORMAL=>'TlwgTypewriter',    FS_BOLD=>'TlwgTypewriter-Bold',  FS_ITALIC=>'TlwgTypewriter-Oblique',   FS_BOLDITALIC=>'TlwgTypewriter-BoldOblique' ),
	    FF_COURIER => array(FS_NORMAL=>'courpro',    FS_BOLD=>'courpro',     FS_ITALIC=>'courpro',    FS_BOLDITALIC=>'courpro' )
	    // Original Fonts.
...
class Image {
    ...
    function GetTTFBBox($aTxt,$aAngle) {
    $box = @ImageTTFBBox($this->font_size,$aAngle,$this->font_file,$aTxt);
	if( $box === false ) {
	    JpGraphError::Raise("Problem reading font file (".$this->font_file."). Make sure file exists and is in a readable place. (If 'basedir' restriction is enabled then it must be located somewhere in the document root.)");
	}
	return $box;
    }
	...
	if( !ereg("\n",$txt) || ($dir>0 && ereg("\n",$txt)) ) {
		...
		$yl = $y  - $yadj;
		$xl = $xl  - $xadj;
		ImageTTFText ($this->img, $this->font_size-2, $dir,
			      $xl, $yl-($h-$fh)+$fh*$i,
	...
	elseif($this->font_family >= FF_FONT1 && $this->font_family <= FF_BOOK)  {
