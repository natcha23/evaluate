...
// Aspera Generic Installer JavaScript Library
// Revision: 2011-12-07
//
// Internal library
...
AW.AsperaInstaller = function() {
	...
	that.currentAppId = "Aspera.Installer.5";
	that.currentMimeType = "application/x-aspera-installer-5";
	that.installerPath = ""; 
	...
	function getConnectReleaseVersion (pluginEl) {
	  ...
	  return version;
					...
					if (browser.is.ie) {
						...
						if (AW.utils.browser.hasMimeType(that.currentMimeType)) {
							...
							if (that.controlNp != null) {
								that.installedVersion = getConnectReleaseVersion(that.controlNp);
							}
						}
					}
				}
		...
		this.updateAvailable = function() {
			...
			if (!that.installedVersion || 
				AW.utils.versionLessThan(that.installedVersion, that.availableVersion))
			{
				return true;
			}
			return false;
		...
		this.shouldRestart = function() {
			var currentVer, plug; 
			var shortenVersionTo = function(versionText, places) {
				try {
			...
			for (var i=0; i<navigator.plugins.length; i++) {
				...
				if (/Aspera Web/.test(plug.name) && plug[0].type === that.currentMimeType) {
					...
					currentVer = shortenVersionTo(plug.version, 3);
					break;
				}
			}
		...
		this.startExternalInstall = function() {
			...
			installerWindow = window.open(AW.utils.joinPaths(that.installerPath, 'install/auto/index.html' + 
				...
				'status=no, titlebar=yes, width=700, height=420, top=12, left=12');
		...
		this.init = function(options) {
			// options:
			// minVersion : If user version is equal to or greater than this version, no upgrade will be shown.
		...
		function parseNavigator() {
		  ...
		  // in connectversions.js.
			...
			if (that.os === "MacIntel" && /Mac OS X 10[._]4/.test(agent) && /Firefox\/3|Safari/.test(agent)) {
					that.os = "MacIntel-legacy";
					that.applet_supported = true;
			}

			if (that.os === "MacIntel" && /Mac OS X 10[._]6/.test(agent) && /Version\/5.0/.test(agent) ) {
					that.applet_supported = false;
			}
		...
		this.getEnvSupportInfo = function () {
			...
			isSupportedOS = (/Win/.test(uPlatform) && !AW.utils.versionLessThan(uAgent.match(/Windows NT (\d.\d)/)[1], '5.1')) ||
				(/Mac/.test(uPlatform) && parseInt(uAgent.match(/OS X 10[._](\d+)/)[1]) >= 4) ||
			...
			envProfiles = {
				...
				// Firefox with Aspera Web but no installer plugin.
				firefoxUpgrade : {
					test: (AW.utils.browser.is.ff && !AW.utils.browser.hasMimeType('application/x-aspera-installer') && 
					AW.utils.browser.hasMimeType('application/x-aspera-web')),
					description : "Upgrade from manual installations on Firefox",
					code: "downloadAndInstall",
					downloadOnly: true,
					isSupported: true
				},

				// IE 7 and IE 8 are buggy when downloading the Connect MSI over HTTPS. 
				IeOverHttps : {
				  test : (location.protocol.indexOf('https') > -1 && (AW.utils.browser.is.ie8 || AW.utils.browser.is.ie7)),
				  description: "IE7 or IE8 over HTTPS",
				  code: "downloadAndInstall",
				  downloadOnly: true,
				  isSupported:true
				},
				...
				isSafari4Plus : {
  				  test : (AW.utils.browser.is.safari && /Mac/.test(uPlatform) && uAgent.match(/Version\/(\d+)/).length > 0 && parseInt(uAgent.match(/Version\/(\d+)/)[1]) >= 4),
  				  description: "Safari",
  				  code: "downloadAndInstall",
  				  downloadOnly: true,
  				  isSupported:true
				...
				isChrome : {
				  test : (uAgent.toLowerCase().indexOf('chrome') > -1),
				...
				safari4MacIntel10_4 : {
					...
					test : ('WebkitAppearance' in docStyle && /Version\/[34]./.test(uAgent) && isMacIntel10_4),
					description : "Safari 4, Mac Intel OS X 10.4",
					platform : 'MacIntel-legacy',
					code: "legacy10_4Safari4_0",
					downloadOnly: true,
					isSupported : true
				},
				...
				linux32 : {
					test: (/Linux i686/.test(uPlatform)),
				...
				linux64 : {
					test: (/Linux x86_64/.test(uPlatform)),
					description : "Please download and run the installer script.",
					code: "linux64",
					downloadOnly: true,
					isSupported : true
