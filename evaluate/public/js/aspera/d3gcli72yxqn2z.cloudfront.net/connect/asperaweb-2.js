...
AW.Connect = function(options) {"use strict";
  ...
  var API_VERSION, DROP_MODE, EVENT, HTTP_METHOD, STATUS, URI_VERSION_PREFIX, appId, firstTransferEventReceived, hostname, id, isEventSupported, listeners, that, supportsNativeDragAndDrop;
   ...
   * 2. `AW.Connect.EVENT.DROP` (`"drop"`)
   * 3. `AW.Connect.EVENT.TRANSFER` (`"transfer"`)
   **/
  AW.Connect.EVENT = {
    ALL : "all",
    DROP : "drop",
  ...
  */
  ...
  function insertPluginObject(options) {
    ...
    if (options.imageCover.length !== 0 && navigator.userAgent.indexOf('MSIE') !== -1) {
 ...
 * download directory setting. Setting this value to `false` overrides this
