			...
			var dt = element.dataTable( {
	         ...
	         // fnServerData 
	            "fnServerData": function ( sUrl, aoData, fnCallback, oSettings ) {
	            	...
	            	var response = service.getProvinceList(page, itemCountPerPage, criteria);
...
//						$(".tooltips").tooltip();
		...
		save: function(data, target) {
			...
			if ($.isEmptyObject(data)) {
				target.find('.modal-title').text(app._('Add Province'));
				target.find(buttonRule).text(app._('Add Province'));
			} else {
				target.find('.modal-title').text(app._('Edit Province'));
				target.find(buttonRule).text(app._('Edit Province'));
			};
			...
			target.on('click', buttonRule, function(e) {
				...
				me.getService().saveProvince(item);
				me.load();
		...
		delete: function(data, target) {
			...
			// lang
			target.find('.modal-body').text(app._('Do you want to delete this province?'));
			...
			target.on('click', '.btn.delete', function() {
				...
				me.getService().deleteProvinceById(data.id);
				me.load();
			});
		},
